...
nb_to_download = [3,6,9] # 保存する画像の枚数
first_image_position = [0,3,6] # 保存を何枚目から始めるか指定
...
def search_and_save(text, number, first_position):
    ...
    for _ in range(number_of_scrolls):
        ...
        time.sleep(2.5)
        try:
            driver.find_element_by_xpath("//input[@value='結果をもっと表示']").click()
            time.sleep(2.5)
    ...
    for img in imges:
        if img_skip < first_position:
            ...
            try:
                ...
                with open(download_path+text.replace(" ", "_")+"/"+str(img_skip+downloaded_img_count)+"."+img_type, 'wb') as f:
                    f.write(raw_img)
